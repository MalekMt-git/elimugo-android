...
class Services @Inject constructor(
    ...
    private val mWiFiDirectManager: WiFiDirectManager = WiFiDirectManager.getInstance(context)
    ...
    fun stop() {
        ...
        if (mWiFiDirectManager.isEnabled) {
            Log.d(TAG, "onDestroy: Stopping hotspot (previously started)=" + mWiFiDirectManager.disable())
        }
    }
    ...
    fun toggleWiFiDirect() {
        if (mWiFiDirectManager.isEnabled) {
            mWiFiDirectManager.disable()
        } else {
            mWiFiDirectManager.enable()
        }
    }
